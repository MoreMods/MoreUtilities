...
package ru.ardev.moreutilities.keybind;
...
public class ClearDropKeyKeyBinding extends MoreutilitiesModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public ClearDropKeyKeyBinding(MoreutilitiesModElements instance) {
		super(instance, 7);
	...
	public void initElements() {
		keys = new KeyBinding("key.moreutilities.clear_drop_key", GLFW.GLFW_KEY_KP_ADD, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					MoreutilitiesMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				ClearDropProcedureProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
